...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for task in notion.tasks:
        ...
        sensor_list.append(
            NotionBinarySensor(notion, task, sensor, bridge, system, name, device_class)
        )
...
class NotionBinarySensor(NotionEntity, BinarySensorDevice):
    ...
    async def async_update(self):
        ...
        new_data = next((t for t in self._notion.tasks if t["id"] == self._task["id"]))
        ...
        if self._task["task_type"] == SENSOR_BATTERY:
            ...
            _LOGGER.error("Unknown binary sensory type: %s", self._task["task_type"])
